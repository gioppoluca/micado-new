...
// so they get immediate feedback that their change was accepted.
watch(
  () => ({ ...createDialog.form }),
...
async function submitCreateDialog(): Promise<void> {
  ...
  try {
    ...
    // Show error inside the dialog (keeps it open so the user can correct the input)
    // and also notify briefly for accessibility
