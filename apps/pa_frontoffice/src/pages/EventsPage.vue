                ...
                <div class="events-header q-mb-md">
                    ...
                    <div class="row items-center q-gutter-sm q-mt-sm">
                        ...
                        <q-btn v-if="!formOpen" color="primary" no-caps unelevated :label="t('button.categories')"
                            @click="$router.push('/data_settings/event_categories')" />
                    </div>
                </div>
...
function toggleTopicFilter(id: number): void {
    ...
    // exactOptionalPropertyTypes: omit topicIds key entirely when empty
    const { topicIds: _removed, ...rest } = activeFilter.value;
    activeFilter.value = next.length
        ? { ...rest, topicIds: next, page: 1 }
        : { ...rest, page: 1 };
    currentPage.value = 1;
    void store.fetchAll(activeFilter.value);
}
...
function toggleUserTypeFilter(id: number): void {
    ...
    // exactOptionalPropertyTypes: omit userTypeIds key entirely when empty
    const { userTypeIds: _removed, ...rest } = activeFilter.value;
    activeFilter.value = next.length
        ? { ...rest, userTypeIds: next, page: 1 }
        : { ...rest, page: 1 };
    currentPage.value = 1;
    void store.fetchAll(activeFilter.value);
}
